...
class StatusHandler:
    ...
    def get_result(self, translation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Pour les traductions réussies
            translation_info = self.state_manager.get_translation_state(translation_id)
            if not translation_info:
                return {
                    "success": False,
                    "message": "Informations de traduction perdues"
                }

            # Préparation du fichier traduit
            result_data = self._prepare_final_result(translation_info, status_data)

            # Nettoyage optionnel des fichiers temporaires
            self._cleanup_after_completion(translation_info)

            logger.info(f"✅ Résultat préparé pour: {translation_id}")
            return {
                "success": True,
                "data": result_data
            }
